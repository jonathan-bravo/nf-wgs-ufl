...
def germline_nextflow(pem, bucket, out_dir, run, exome):
    ...
    commands = [
        f"git -C /nf-wgs-ufl/ pull; nextflow run /nf-wgs-ufl/main.nf -work-dir s3://{bucket}/{out_dir}/_work/ --bucket 's3://{bucket}' --run_id '{run}' --single_lane '{single_lane}' --match '{match}' --exome '{exome}' --run_dir 's3://{bucket}/{out_dir}/{run}' -resume"
    ]
